...
struct table tableSlave(struct dirent * file_d, std::string dir_s) {
  ...
  std::string line;
  std::string img = "/XY/icons/unknown.gif";
  std::string type = "   ";
...
std::string dirSlave(std::string dir_s) {
  SortType st;
  // prepare SortType
  std::size_t found = dir_s.find("/?");
  if (found != std::string::npos) {
    std::string sort_request = dir_s.substr(found + 2);
    dir_s = dir_s.substr(0, found+1);
  ...
  std::string sort = 
    "<tr>"
    "<th valign=\"top\"><img src=\"/XY/icons/index.gif\" alt=\"[ICO]\"></th>"
